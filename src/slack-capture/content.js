  ...
  scriptFirestore.onload = () => {
    ...
    const observeMessageChanges = () => {
      ...
      if (messageObserver) {
        ...
        console.log(
          "Slack Text Logger: 既存のMutationObserverを切断しました。"
        );
      }
    ...
    const displayCapturedMessage = (message) => {
      try {
        ...
        const contentDiv = displayDiv.querySelector(
        ...
        // メッセージとボタンをラップするdivを作成
        const messageWrapper = document.createElement("div");
        messageWrapper.style.display = "flex";
        messageWrapper.style.alignItems = "center"; // 垂直方向に中央揃え
        messageWrapper.appendChild(messageElement);
        messageWrapper.appendChild(sendButton);

        // ラップしたdivをcontentDivに追加
    ...
    const createDisplayDiv = () => {
      console.log(
        "Slack Text Logger: createDisplayDiv 関数が呼び出されました。"
      );
    ...
    window.addEventListener("load", () => {
      console.log("Slack Text Logger: ページロード完。初期化を開始します。");
      initialize();
    });
  };
